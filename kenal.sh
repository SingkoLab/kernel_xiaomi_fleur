...
git commit -sm "BEGIN Xiaomi Imports" --allow-empty --amend >/dev/null 2>&1
...
for ELEMENT in ${REMNANTDIRS[@]}; do
	[[ -d $ELEMENT ]] && git add $ELEMENT -f >/dev/null 2>&1
	git commit -sm "$ELEMENT: import rest of the Xiaomi changes" >/dev/null 2>&1
done

git add -- . :!rebase.sh :!out >/dev/null 2>&1
git commit -sm "kernel: import rest of misc Xiaomi changes" >/dev/null 2>&1

echo "Committing End point"
git commit -sm "END Xiaomi IMPORTS" --allow-empty >/dev/null 2>&1
