...
static int fsm_ext_reset(fsm_dev_t *fsm_dev)
{
	...
	//	return 0;
	//}
	pr_info("fsm_dev->rst_gpio[%d]: %d",fsm_dev->id,fsm_dev->rst_gpio);
			...
			const struct i2c_device_id *id)
{
	...
	if (i2c->addr != 0x34) {
		fsm_dev->addr = fsm_dev->i2c->addr = 0x34;
		fsm_reg_write(fsm_dev, 0x10, 0x0002); // reset nack
		fsm_reg_read(fsm_dev, 0x10, NULL); // dummy read
		fsm_delay_ms(15); // 15ms
		fsm_dev->addr = fsm_dev->i2c->addr = 0x35;
	}
